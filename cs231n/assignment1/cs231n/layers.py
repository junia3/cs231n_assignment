...
def svm_loss(x, y):
    ...
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    N = x.shape[0]
    correct_x = x[np.arange(N), y].reshape(N, 1)
    margin = np.maximum(0, x - correct_x + 1)
    margin[np.arange(N), y] = 0
    loss = np.sum(margin) / N

    dx = np.where(margin > 0, 1, margin)
    count_plus = np.sum(dx, axis = 1)
    dx[np.arange(N), y] -= count_plus
    dx /= N
...
def softmax_loss(x, y):
    ...
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    N = x.shape[0]
    logs = np.exp(x - np.max(x))
    value = logs / np.sum(logs, axis = 1, keepdims = True)
    loss = -np.sum(np.log(value[np.arange(N), y])) / N
    dx = value.copy()
    dx[np.arange(N), y] -= 1
    dx /= N
